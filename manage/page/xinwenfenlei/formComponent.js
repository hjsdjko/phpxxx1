Vue.component('xinwenfenlei-form', {
template:`
<template>
    <div>
        <el-dialog
            ...
            <el-form class="formModel_form" ref="formRef" :model="form" :rules="rules" >
                <el-form-item  label="新闻分类" prop="xinwenfenlei" class="input-item">
                    <el-input v-model="form.xinwenfenlei"
                       placeholder="新闻分类"
                       type="text"
                       :readonly="!isAdd||disabledForm.xinwenfenlei?true:false" ></el-input>
                </el-form-item>
            </el-form>
...
methods: {
    ...
    getInfo(){
        http.get(`xinwenfenlei/info/${this.id}`).then(res=>{
    ...
    save(){
        ...
        this.$refs.formRef.validate((valid)=>{
            ...
            if(crossUserId&&crossRefId){
                ...
                http.get('xinwenfenlei/page',{
                    params:params
                }).then(res=>{
                    if(res.data.data.total>=crossOptNum){
                        return this.$message.error(this.crossTips)
                    }else{
                        http.post(`xinwenfenlei/${!this.form.id ? "save" : "update"}`,this.form).then(res=>{
                            if(this.type == 'cross'){
                                //修改跨表数据
