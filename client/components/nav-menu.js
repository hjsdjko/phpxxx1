...
<div id="nav-menu">
    <el-menu mode="horizontal" >
        <el-menu-item @click="location.href=baseUrl+'client/page/index.html'">首页</el-menu-item>
        <template v-for="(item,index) in indexMenuList" :key="item.name">
            <el-menu-item v-if="item.child.length==1" @click="navigateTo(item.child[0])">{{item.child[0].name}}</el-menu-item>
            <el-submenu v-else>
                <template slot="title">{{item.name}}</template>
                <el-menu-item v-for="item1 in item.child" :key="item1.name" @click="navigateTo(item1)">
                    {{item1.name}}
                </el-menu-item>
            </el-submenu>
        </template>
